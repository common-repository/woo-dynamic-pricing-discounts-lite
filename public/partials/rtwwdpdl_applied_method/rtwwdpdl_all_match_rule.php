...
$rtwwdpdl_bogo_text = 'Buy [quantity1] [the-product] Get [quantity2] [free-product] at [free-pro-disc-val]% Off';
				...
				if (is_array($rtwwdpdl_rule_name) && !empty($rtwwdpdl_rule_name))
				{
					...
					foreach ($rtwwdpdl_rule_name as $name)
					{
						...
						if ($rtwwdpdl_date > $rtwwdpdl_today_date)
						{
							...
							{
								// update 1.6.2 Apply discount whether selected for parent or subcategories
								$cat_with_id = explode('_', $name['category_id']);
								$cat = $cat_with_id[1];
								$rtwwdpdl_cate_to_find = array();
								if ($cat_with_id[0] == 'subcate')
								{
									array_push($rtwwdpdl_cate_to_find, $rtwwdpdl_product_cat_id);
								}
								else
								{
									$rtwwdpdl_cate_to_find[] = $rtwwdpdl_product_cat_id;
									$rtwwdpdl_cate_to_find = array_merge($rtwwdpdl_cate_to_find, get_ancestors($rtwwdpdl_product_cat_id, 'product_cat'));
								}

								// update 1.6.2 Apply discount whether selected for parent or subcategories (end)

								if (in_array($cat, $rtwwdpdl_cate_to_find) && $rtwwdpdl_match == false)
								{
							...
							if (isset($name['product_id']) && is_array($name['product_id']) && !empty($name['product_id']))
							{
								foreach ($name['product_id'] as $no => $ids)
								{
									if ($ids == $rtwwdpdl_prod_id && $rtwwdpdl_match == false)
									{
										...
										// ========== update 1.6.2 =============//									
										$rtwwdpdl_bogo_text = str_replace('[free-pro-disc-val]', !empty($name['rtwwdpdl_bogo_prcnt_off']) ? $name['rtwwdpdl_bogo_prcnt_off'] : '100', $rtwwdpdl_bogo_text);
										// ========== update 1.6.2 =============//
