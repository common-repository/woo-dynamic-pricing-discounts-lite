									...
									foreach ($this->rtwwdpdl_priority as $kval)
									{
										if ($kval == 'pro_rule_row')
										{
											...
											foreach ($rtwwdpdl_pro_rul as $pro => $rul)
											{
												...
												foreach ($rtwwdpdl_temp_cart as $cart_item_key => $cart_item)
												{
													...
													$rtwwdpdl_cart_pro_id = $product->get_id();
													...
													if ($rtwwdpdl_original_price)
													{
														...
														if ($rul['rtwwdpdl_rule_on'] == 'rtwwdpdl_products' || $rul['rtwwdpdl_rule_on'] == 'rtwwdpdl_cart')
														{
															if ($rul['rtwwdpdl_rule_on'] != 'rtwwdpdl_cart')
															{
																if ($rul['product_id'] != $rtwwdpdl_cart_pro_id)
																{
																	continue 1;
																}
															}
															...
															{
																if (!empty($rul['rtwwdpdl_min']) && $cart_item['quantity'] < $rul['rtwwdpdl_min'] || !empty($rul['rtwwdpdl_max']) && $cart_item['quantity'] > $rul['rtwwdpdl_max'])
																{
																	continue 1;
																}
															}
															...
															{
																if (!empty($rul['rtwwdpdl_min']) && $cart_item['data']->get_price() < $rul['rtwwdpdl_min'] || !empty($rul['rtwwdpdl_max']) && $cart_item['data']->get_price() > $rul['rtwwdpdl_max'])
																{
																	continue 1;
																}
															}
														...
														{
															// ============= update for multiple product (only for OR condition) ============//
															// if (isset($rul['rtwwdpd_condition']) && $rul['rtwwdpd_condition'] == 'rtwwdpd_and')
															// {
															// 	// $total_quantities = array();
															// 	// $total_prices = array();
															...
															// else
															// {	
															if (!in_array($rtwwdpdl_cart_pro_id, $rul['multiple_product_ids']))
															{
																continue;
															}
															if ($rul['rtwwdpdl_check_for'] == 'rtwwdpdl_quantity')
										...
										elseif ($kval == 'cat_rule_row')
										{
											...
											foreach ($rtwwdpdl_pro_rul as $pro => $rul)
											{
												...
												$cat_with_id = explode('_', $rul['category_id']);
												$cat = $cat_with_id[1];
												foreach ($rtwwdpdl_temp_cart as $cart_item_key => $cart_item)
												{
													// update 1.6.2 for apply discount if its parent cat is selected to get discount.

													$rtwwdpdl_item_terms = get_the_terms($cart_item['data']->get_id(), 'product_cat');
													foreach ($rtwwdpdl_item_terms as $term)
													{
														if ($cat_with_id[0] == 'subcate')
														{
															$rtwwdpdl_cat_ids_dta[] = $term->term_id;
														}
														else
														{

															$rtwwdpdl_cat_ids_dta[] = $term->term_id;
															$rtwwdpdl_cat_ids_dta = array_merge($rtwwdpdl_cat_ids_dta, get_ancestors($term->term_id, 'product_cat'));
														}
													}
													...
													// }													
													if (!empty($rtwwdpdl_cat_ids_dta) && in_array($cat, $rtwwdpdl_cat_ids_dta))
													{
												...
												foreach ($rtwwdpdl_temp_cart as $cart_item_key => $cart_item)
												{
													...
													// foreach ($rtwwdpdl_item_terms as $term)
													// {
													// 	$rtwwdpdl_catids = ($term->parent != '0') ? $term->parent : $term->term_id;
													// }

													$categories = get_the_terms($cart_item['data']->get_id(), 'product_cat');
													$cat_ids = array();
													foreach ($categories as $category)
													{
														...
														else
														{
															$cat = $cat_with_id[1];
															$cat_ids[] = $category->term_id;
															$cat_ids = array_merge($cat_ids, get_ancestors($category->term_id, 'product_cat'));
														}
													}

													if (isset($rul['category_id']) && !empty($rul['category_id']) && !in_array($cat, $cat_ids))
													{
														continue 1;
													}
													// update 1.6.2 for apply discount if its parent cat is selected to get discount.
													...
													if ($rul['rtwwdpdl_check_for_cat'] == 'rtwwdpdl_quantity')
													{
														...
														if (isset($rul['rtwwdpdl_max_cat']) && $rul['rtwwdpdl_max_cat'] != '')
														{
															if ($rul['rtwwdpdl_max_cat'] < $rtwwdpdl_total_quantity)
															{
																continue;
															}
														}
													...
													elseif ($rul['rtwwdpdl_check_for_cat'] == 'rtwwdpdl_price')
													{
														...
														if (isset($rul['rtwwdpdl_max_cat']) && $rul['rtwwdpdl_max_cat'] != '')
														{
															if ($rul['rtwwdpdl_max_cat'] < $rtwwdpdl_total_price)
															{
																continue;
															}
														}
													...
													else
													{
														...
														if (isset($rul['rtwwdpdl_max_cat']) && $rul['rtwwdpdl_max_cat'] != '')
														{
															if ($rul['rtwwdpdl_max_cat'] < $rtwwdpdl_total_weight)
															{
																continue;
															}
														}
										...
										elseif ($kval == 'tier_rule_row')
										{
											...
											foreach ($rtwwdpdl_pro_rul as $pro => $rul)
											{
												...
												$rtwwdpdl_user_role = isset($rul['rtwwdpdl_select_roles']) ?  $rul['rtwwdpdl_select_roles'] : array('all');
