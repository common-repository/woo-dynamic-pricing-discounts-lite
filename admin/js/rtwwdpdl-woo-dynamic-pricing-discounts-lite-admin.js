(function ($) {
	...
	$(document).on('click', '#rtwinsertbtnbogo', function () {
		...
		var quant = '<input type="number" min="0" name="combi_quant[]" required value=""  />';
	...
	$(document).on('click', '#rtwinsert_bogo_pro', function () {
		...
		var quant = '<input type="number" min="0" name="bogo_quant_free[]" required value=""  />';
		var disc_val = '<input type="number" value="" min="0" step="0.01" name="rtwwdpdl_bogo_prcnt_off" /> <span style="display:block"><i>Default Discount (100 %).</i></span>';
	
		var remove = '<a class="button insert remove" name="deletebtn" >Remove</a>';
		$('#rtw_bogo_row').append('<tr><td>' + row_no + '</td><td>' + select + '</td><td>' + quant + '</td><td>' + disc_val + '</td><td>' + remove + '</td></tr>');
	...
	// ========================= update =========================// 
	setTimeout(function () {
		$(document).find('.rtwwdpdl_auto_remove_notice').remove();
	}, 3000);
	// ========================= update =========================// 
